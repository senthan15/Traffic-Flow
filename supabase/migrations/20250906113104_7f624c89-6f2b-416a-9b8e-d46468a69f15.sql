...
-- Admin policies for modifications (only authenticated users with admin role can modify)
CREATE POLICY "Authenticated users can modify intersections" ON public.intersections FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can modify traffic data" ON public.traffic_data FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can modify signal timing" ON public.signal_timing FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated users can modify traffic events" ON public.traffic_events FOR ALL USING (auth.role() = 'authenticated');
...
-- Insert some traffic events
INSERT INTO public.traffic_events (intersection_id, event_type, description, severity, status) 
SELECT 
  i.id,
  (ARRAY['incident', 'maintenance', 'congestion', 'emergency'])[1 + (RANDOM() * 4)::INTEGER],
  CASE 
  ...
  END,
  (ARRAY['low', 'medium', 'high'])[1 + (RANDOM() * 3)::INTEGER],
