...
const Analytics = () => {
  ...
  const exportToPDF = () => {
    ...
    // Add title
    doc.setFontSize(20);
    doc.text('Traffic Analytics Report', 20, currentY);
    currentY += 20;
    ...
    const keyMetrics = [
      ['Metric', 'Value', 'Note'],
      ['Average Wait Time', '48s', '12% improvement this week'],
      ['Daily Vehicles', '2,847', 'Average across all intersections'],
      ['Peak Hour Efficiency', '54%', '5:00-6:00 PM rush hour'],
      ['Traffic Violations', '11', 'Red light violations today']
    ];
    ...
    // Add intersection performance section
    doc.setFontSize(16);
    doc.text('Intersection Performance', 20, currentY);
    currentY += 10;
    
    const intersectionTableData = [
      ['Intersection', 'Avg Wait Time (s)', 'Violations'],
      ...intersectionData.map(item => [item.name, item.avgWait.toString(), item.violations.toString()])
    ];
    
    autoTable(doc, {
      head: [intersectionTableData[0]],
      body: intersectionTableData.slice(1),
      startY: currentY,
      theme: 'grid',
      headStyles: { fillColor: [59, 130, 246] },
      styles: { fontSize: 10 },
    ...
    doc.setFontSize(16);
    doc.text('System Status Distribution', 20, currentY);
    currentY += 10;
    
    const statusData = [
      ['Status', 'Percentage'],
      ...signalStatusData.map(item => [item.name, `${item.value}%`])
    ];
    ...
    // Add hourly traffic summary if there's space
    if (currentY < 220) {
      doc.setFontSize(16);
      doc.text('Hourly Traffic Summary', 20, currentY);
      currentY += 10;
      
      const hourlyData = [
        ['Time', 'Vehicles', 'Efficiency %'],
        ...hourlyTraffic.slice(0, 6).map(item => [item.hour, item.vehicles.toString(), `${item.efficiency}%`])
      ];
    ...
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      doc.setFontSize(8);
      doc.text(`Generated by Smart Traffic Management System - Page ${i} of ${pageCount}`, 20, (doc as any).internal.pageSize.height - 10);
    }
    
    // Save the PDF
    const fileName = `traffic-analytics-${format(new Date(), 'yyyy-MM-dd-HHmm')}.pdf`;
    doc.save(fileName);
